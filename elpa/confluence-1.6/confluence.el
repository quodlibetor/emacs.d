...
(defvar confluence-prefix-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "^f") 'confluence-get-page)
    (define-key map (kbd "^c") 'confluence-create-page)
    (define-key map (kbd "^=") 'confluence-ediff-current-page)
    (define-key map (kbd "^m") 'confluence-ediff-merge-current-page)
    (define-key map (kbd "^p") 'confluence-get-parent-page)
    (define-key map (kbd "^r") 'confluence-rename-page)
    (define-key map (kbd "^s") 'confluence-search)
    (define-key map (kbd "^.") 'confluence-get-page-at-point)
    (define-key map (kbd "^*") 'confluence-pop-tag-stack)
    (define-key map (kbd "^v") 'confluence-preview)
    (define-key map (kbd "^a") 'confluence-get-attachment)
    (define-key map (kbd "^b") 'confluence-browse-page)
    (define-key map (kbd "^x") 'confluence-get-related-page)
    (define-key map (kbd "^j") confluence-format-prefix-map)
